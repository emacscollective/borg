...
Now that ~borg~ is available we can build all the assimilated packages
(currently just ~borg~ itself) using ~make bootstrap~.

Next we have to tell Emacs to initialize Borg instead of Package, by
