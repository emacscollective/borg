...
usually does its work using ~borg-update-autoloads~, ~borg-compile~, and
~borg-makeinfo~.
...
- Variable: submodule.DRONE.build-step COMMAND ::
  ...
  ~borg-update-autoloads~, ~borg-compile~, and ~borg-makeinfo~, but if this
  variable has one or more values, then DRONE is built using these
  COMMANDs *instead*.
...
- Variable: borg-compile-recursive ::
