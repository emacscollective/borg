...
This manual is for Borg version 4.3.0.
...
- Function: borg-report-after-init-duration ::

  This function reports how long it takes to run ~after-init-hook~.
  This is done by adding two functions to that hook; one with a very
  low depth and the other with a very high depth.  Calling this
  function does nothing if the hook already ran.
