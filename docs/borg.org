...
itself, namely ~magit~, ~epkg~, ~use-package~, ~auto-compile~ and ~git-modes~,
as well as their dependencies.  These drones are not required by ~borg~
but their use is highly recommended.

Clone the ~emacs.g~ repository to either ~~/.config/emacs~, ~~/.emacs.d~ or
for testing purposes to any other location.  Below we assume you used
the former, so otherwise you'll have to substitute your choosen path
for that in the examples below.
...
#+begin_src sh
  git clone https://github.com/emacscollective/emacs.g ~/.config/emacs
  cd ~/.config/emacs
  make bootstrap-borg
...
the ~emacs.g~ collective uses the ~https~ protocol by default, but your can
change that on the fly using the following global rules.
