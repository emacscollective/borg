...
packages are distributed.  Instead copy the URLs from this list.
...
dependencies".  Next you should assimilate ~auto-compile~ and create
a commit.  Then you should assimilate ~magit~ and its dependencies.
Finally you should configure Magit to list submodules in status
buffers:
