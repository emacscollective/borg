...
use that configuration using ~emacs -Q --load /path/to/emacs.g/init.el~.
...
To determine the drones which you /might/ want to update, visit the
Magit status buffer of the ~~/.config/emacs~ repository and press ~f m~ to
fetch inside all submodules.  After you have done so, and provided
there actually are any modules with new upstream commits, a section
titled "Modules unpulled from @{upstream}" appears.
...
creating a release, you should record the changes in the
~~/.config/emacs~ repository.  To do so proceed as described in
[[*Updating drones]].
...
- Command: native ::
  ...
  it also byte-compiles the user init files, like ~build-init~ does.
