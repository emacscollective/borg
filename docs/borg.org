...
- Command: bootstrap ::
  ...
  If an error occurs during ~make clone-modules~ or ~make
  checkout-modules~, then you can just run the target that failed (and
  the subsequent targets) again to process the remaining drones.  The
  drones that have already been successfully processed and drones for
  which an earlier stop has failed will be skipped.
...
- Command: checkout-modules ::
  ...
  that were successfully cloned by ~make clone-modules~, while also
  making sure that a branch is checked out for every module.

  It is safe to rerun this target.  Modules whose HEAD is no longer
  detached (likely because an earlier run took care of that), as well
  as modules that have uncommitted changes, are skipped.

  Alternatively drones can be checked out individual using
  ~lib/borg/borg.sh checkout [--force] DRONE~.  Use ~--force~ if HEAD is
  no longer detached but you want to perform this step again anyway.
