...
#+SUBTITLE: for version 1.0.0 (v1.0.0-38-g202a380+1)
...
#+BEGIN_QUOTE
This manual is for Borg version 1.0.0 (v1.0.0-38-g202a380+1).
...
- Variable: submodule.DRONE.build-step COMMAND
  ...
  By default drones are build using the lisp functions
  ~borg-update-autoloads~, ~borg-byte-compile~, and ~borg-makeinfo~, but
  if this variable has one or more values, then DRONE is build using
  ...
  it is assumed to be a shell command and excecuted with ~shell-command~.
...
- Variable: borg-build-shell-command
  ...
  If the value returned by the first function, then this second
  function is called for each build step with the drone and the build
  step as arguments.  It must return a string or ~nil~.  If the returned
  value is a string, then that is used as described above.
